...
public class ExpertAnimator : MonoBehaviour, IOnSensorCollision {
	...
	public ExpertAnimator GetFirstOfThisAnim()
	{
		if (isFirstOfThisAnim)
			return this;
		var anim = GetComponent<Animator>();
		var styleAnimators = FindObjectsOfType<ExpertAnimator>().ToList();
	...
	void UpdateAnimationStep(float timeStep)
    {
		...
		animStep.CenterOfMass = JointHelper.GetCenterOfMassRelativeToRoot(BodyParts);
		...
		foreach (var bodyPart in BodyParts)
		{
			...
			if (NormalizedTime != 0f) {
				...
				animStep.AngularVelocities[i] = JointHelper.CalcDeltaRotationNormalizedEuler(_lastRotation[i], bodyPart.Transform.rotation) / (_decisionRequester.DecisionPeriod * Time.fixedDeltaTime); ;
				...
				animStep.AngularVelocitiesLocal[i] = JointHelper.CalcDeltaRotationNormalizedEuler(_lastRotationLocal[i], animStep.Rotations[i]) / (_decisionRequester.DecisionPeriod * Time.fixedDeltaTime);
			}

			if (bodyPart.Rigidbody != null) {
				bodyPart.Rigidbody.angularVelocity = JointHelper.CalcDeltaRotationNormalizedEuler(bodyPart.Transform.rotation, _lastRotation[i]) / (_decisionRequester.DecisionPeriod * Time.fixedDeltaTime);
		...
		animStep.AngularMoment = JointHelper.GetAngularMoment(BodyParts);
